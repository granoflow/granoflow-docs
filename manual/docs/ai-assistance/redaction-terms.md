...
这张表只保存你手动维护的固定替换规则。系统按规则自动发现的邮箱、金额、链接、日期、长数字、银行卡、IBAN、电话等内容，会按 AI 脱敏设置里的类别默认策略临时替换成更容易读懂的短期脱敏值，例如 `13xxxxx4821` 或 `foxxxx3920@1846.com`，不会自动写进这里的长期词表。
...
- 只处理你已经设置的固定词；规则型自动发现会走短期脱敏值，不会替你维护长期词表。
