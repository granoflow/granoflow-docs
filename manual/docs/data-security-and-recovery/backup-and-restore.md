...
- 删除大量任务或项目之前
- 完成一个重要阶段后，想保留当时的记录
- 导入外部卡组、整理大量复习卡片或清理附件缓存之前

## 卡组包与完整备份

数据管理页以卡片展示几类不同的数据操作。`本地备份` 卡片创建 `.flow.grano` 文件，适合换机、重装和整机恢复；`卡组` 卡片处理 `.deck.grano` 卡组包，可迁移选定卡组、卡片和可打包的本地图片媒体，但不能替代完整备份，也不会自动同步到云端。卡组卡片还会显示当前卡片缓存占用与上限，并提供清空缓存入口。更多边界阅读 [卡组、导入与导出](/manual/review-cards/decks-import-export/)。

在 `卡组` 卡片点「导出卡组」会进入卡组列表，由你在列表里选定顶层卡组后再导出。
...
5. 在导入前确认设备已接通电源，尤其是移动设备。
6. 查看备份时间、备份版本和内容摘要，确认这就是你要导入的备份。
7. 如果 GranoFlow 要求输入“数据密钥”，输入这份备份创建时保存的那一把。
8. 确认导入后，等待流程完成，不要在处理中重复操作。

导入时，GranoFlow 会先尝试用当前设备已保存的密钥打开备份。能打开时，不会额外要求你输入；打不开时，才会进入数据密钥输入。这里要输入的是创建这份备份时抄录的数据密钥，不是登录密码，也不是随手新生成的数据密钥。输入错误不会改变当前数据。

如果这台设备已经加入过云端同步，导入备份时 GranoFlow 会尽量保持这台设备和当前账号的同步保护关系不变：备份里的内容会被带入当前设备，而不是让一个旧备份改变当前账号的云端同步身份。如果这台设备从未加入过云端同步，备份会成为这台设备接下来继续使用的基础。你不需要判断这些技术差异；导入过程中只要按提示输入对应的数据密钥，并等待进度完成即可。

导入备份是增量合并，不会清空当前设备。备份包里本机没有的记录会导入；同一条记录会按最后修改时间比较，谁更新就保留谁。如果最后修改时间精确到毫秒后仍然相同，GranoFlow 会采用备份包里的版本，让多台设备更容易收敛到同一份数据。备份包里没有的本机旧记录和本机旧附件会继续保留。

附件也会跟随导入计划处理。父任务、项目或里程碑不存在、已删除，或附件文件本身缺失时，相关附件不会被硬塞进来。如果备份包缺少附件文件，默认会阻止导入；只有在你明确选择“忽略缺失附件继续导入”后，GranoFlow 才会带着这个缺口导入其余可导入记录。这个选择适合你已经确认缺失附件不重要，或只想尽快导入任务、项目和文本记录的情况。

如果导入完成后显示“有若干条数据无法导入”，这些通常是缺少最后修改时间、父链不安全或附件文件不完整的问题数据。GranoFlow 会跳过它们并以本机为准。你可以导出一份明文 JSON 查看这些错误数据；如果其中有你确实需要的内容，只能参考 JSON 手工补录。

:::caution[导入前仍建议先备份当前设备]
导入备份不会清空当前设备，但它可能更新同 ID 且备份较新的记录。导入旧备份前，如果你想给当前状态留一条退路，请先创建一份当前备份，并保存好这份新备份的数据密钥。
:::
