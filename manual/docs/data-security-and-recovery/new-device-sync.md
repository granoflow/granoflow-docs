...
如果你保存的是第一次同步时抄录的数据密钥，它通常就是这套云端数据的云端同步密钥。如果你后来改过数据密钥，要用当前云端同步数据对应的那一把。某个本地备份的数据密钥只有在它和云端同步保护本来就是同一串时，才能用于新设备同步。

最稳妥的顺序是：先在旧设备确认数据还在，再复制或记录同步密钥，最后操作新设备。

<!-- manual-screenshot:id=data-new-device-sync-old-device-key -->
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-new-device-sync-old-device-key.png)
...
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-new-device-sync-enter-key.png)

<!-- manual-screenshot:id=data-new-device-sync-join-existing -->
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-new-device-sync-join-existing.png)

<!-- manual-screenshot:id=data-new-device-sync-restored-data -->
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-new-device-sync-restored-data.png)
...
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-sync-device-join.png)

这个页面通常会从账号/设置里的同步入口，或顶部/侧栏的同步异常提醒进入。它不是普通的同步按钮，也不是“已同步”状态提示，而是在问你要保留哪一边的数据。
...
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-new-device-sync-local-image-task.png)
...
![新设备同步已有云端数据 界面截图](../../screenshots/zh-cn/data-new-device-sync-local-data-choice.png)
...
先检查有没有复制完整，尤其是开头、结尾和空格。确认你输入的是云端同步密钥，不是账号密码，也不是随手新生成的本地数据密钥。备份数据密钥只有在它和当前云端同步保护本来就是同一把时才适用。
