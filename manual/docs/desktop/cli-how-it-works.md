...
description: "理解本机 HTTP API 的监听地址、端口配置、Token 认证与 App 保护门禁。"
---
...
本机 HTTP 配置只保存 schema、protocol、host、port 等非敏感字段，不保存 token、nonce、App lock 状态、账号或跨设备凭据。
...
2. App lock（需要先解锁 App）
3. nonce
4. Token Verification（仅在开启时需要 API token）
