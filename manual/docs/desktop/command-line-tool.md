---
title: "命令行工具"
description: "在桌面端使用 GranoFlow CLI 管理任务、项目、里程碑、标签、领域价值观、回顾和本地备份。"
---

GranoFlow CLI 是给桌面端用户、脚本和 AI 助手使用的命令行入口。它适合批量查看或更新任务、项目、里程碑、标签、领域价值观、回顾记录，以及创建或恢复本地备份。

本页里的 CLI 只指安装到用户电脑上的 `granoflow` 命令。它不是开发、构建或云端运维入口，不提供云端管理员、购买平台管理员、测试账号重置、云端清空或发布类命令。
