...
在 macOS 上，先在 App 的「命令行工具」设置页点「安装命令行工具」或「修复命令行工具」；系统会弹出管理员授权框（Helper 显示名为「Granoflow 命令行工具」），输入登录密码后才会创建 `/usr/local/bin/granoflow` 符号链接。其他平台在同一设置页完成安装或修复后，执行：
