...
description: "Understand the local HTTP API listen address, port configuration, token authentication, and app protection gates."
...
The local HTTP configuration stores only non-sensitive fields (schema, protocol, host, port). It does not store token, nonce, app lock state, account, or cross-device credentials.
...
2. App lock (app must be unlocked first)
3. nonce
