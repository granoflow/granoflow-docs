...
领域页用来维护长期方向和价值观，不是必须一次填完的分类系统。你可以从首页引导、侧栏管理入口或相关设置进入领域管理，然后逐个领域改名、调整副标题，并添加、修改或删除几条价值观。

The page shows a few common domains first as starting points. They are suggestions: you can rename them, adjust their subtitles, add values under them, or use the “New domain” button below the defaults. Domains you create yourself appear below that button.

<!-- manual-screenshot:id=value-domains-management -->
![先确定长期方向 screen capture](../../../screenshots/en/value-domains-management.png)

如果截图没有加载，也不影响理解。你可以把这个页面想成一个“方向整理页”：每个领域下面都有自己的价值观列表；页面会用一些提示问题，帮助你把抽象念头写成一句完整的话，而不是只留下一个漂亮但难用的词。

Each domain card has an AI helper entry. It copies that domain's name, current values, and value limit so an external AI discusses only this domain. AI can help you talk through which projects belong here, what values feel true to you, and which books may be useful reference material; it does not decide your direction for you. What gets saved still depends on your judgment in the domain page.
