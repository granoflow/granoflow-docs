...
**Add card** starts on the **note page**: enter the title first (other fields unlock afterward); title and field content auto-save with no **Save note** button, and a brief **Saved** indicator appears next to the title after each successful save. You can also clear the title on a saved note to lock the other fields again. After title and body, open the **layout page** to place note fields on the front and back. After layout, you return to the note page. The footer shows **Create card layout** when the note has no cards yet, and only **Add another layout** once sibling cards exist. Cards under this note appear as a front-only thumbnail grid; tap a thumbnail to preview the front, then the back, then open layout editing. Advanced options let you edit title translation, content translation, and add custom fields. Source is written automatically from the task name on first title save but is not shown on the note page; it appears as a subtle footnote at the bottom of the card back during review. The task cards area groups sibling cards by note; at group level you can edit the note or unlink the group, and at card level you can edit layout, archive, or move to trash.
...
If a card belongs to a note, the review page shows a note entry. Opening it displays the note title, content, matching translations, and all cards under that note in a right-side panel. Read the question first, reveal the answer, then rate yourself with Forgot, Hard, Good, or Easy. After revealing the answer, move the card to the recycle bin if it is no longer worth studying, or archive it if you only want to pause active review. Archived cards may still appear in related task and review contexts and are labeled archived. These actions offer undo and only affect the card, not the note.
