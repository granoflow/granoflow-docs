...
在支持 Markdown 的字段里点击描述或内容区域，会进入编辑页。编辑页默认是所见即所得：你直接在正文里输入、选中文字加粗或斜体、插入链接，保存时 GranoFlow 仍会把内容保存成 Markdown。

顶部常用按钮包括：

| 按钮 | 用途 |
| --- | --- |
| 源码 | 查看和编辑当前字段的 Markdown 源码，适合排查格式问题或粘贴复杂片段 |
| 插入 | 插入表格、分割线、图片链接、媒体链接、公式源码或图表源码 |
| 保存 | 保存当前内容并返回上一页 |

在宽屏上，常用格式按钮会出现在编辑区上方；在手机或窄屏上，它们会贴近底部，方便单手编辑。

<!-- manual-screenshot-needed:id=markdown-wysiwyg-editor; reason=展示所见即所得编辑器、源码按钮、插入按钮和保存按钮 -->

## 添加表格

点击顶部「插入」按钮，再选「表格」，GranoFlow 会插入一个可编辑表格。插入后直接点进单元格输入文字；保存时表格仍会转换为标准 Markdown 表格。

如果你更习惯手写，也可以点「源码」直接输入 Markdown 表格：
...
<!-- manual-screenshot-needed:id=markdown-table-insert; reason=展示插入菜单、可编辑表格单元格和源码里的 Markdown 表格 -->
...
可以用「插入」里的「公式源码」先放入模板，再在源码窗口里替换成自己的公式。公式内容会作为 Markdown 源码保存。不同平台的公式渲染能力可能随版本逐步开放；如果当前版本或当前页面尚未渲染 LaTeX，公式不会丢失，通常会以源码或普通文本形式显示。遇到公式显示异常时，先点「源码」检查 `$`、`$$` 是否成对。
...
GranoFlow 会把 Mermaid 代码块识别为图表 trigger。点击后在独立视图里查看图表；如果当前版本或平台无法渲染图表，源码仍会保留，你可以点「源码」继续编辑。

也可以用「插入」里的「图表源码」先放入模板。Mermaid 对缩进、箭头和节点字符比较敏感。图表打不开时，先点「源码」检查代码块第一行是不是 ` ```mermaid `，最后一行是不是单独的 ` ``` `，再检查节点名称里是否混入了没有配对的括号或引号。
...
格式不对时，先点顶部「源码」按钮。大多数问题都可以从源码看出来：
