---
title: "账号、同步与数据入口"
description: "了解设置页中的账号、同步、数据、订阅、AI 助手和标签管理入口分别应该如何理解。"
---

如果你在设置页看到账号、同步、数据、订阅、AI 助手或标签管理入口，可以把它们当成通往专门设置页的按钮：账号管登录和设备， 同步管多设备记录一致，数据管导入导出、备份和恢复，订阅管 Pro 权益，AI 助手和标签管理分别管外部 AI 工具配合方式和任务标签。
...
账号入口用来注册、登录、退出、查看账号状态，并把当前设备接入同一个账号体系。
...
同步入口用来让任务、项目、回顾等核心记录在多台设备之间保持一致。
...
数据入口用来导入、导出、备份、恢复、查看附件状态，或清理本地占用。

这些操作通常比外观设置和当前设备偏好更敏感。备份是为了在换机、重装或出现异常时保留重要数据；恢复会把备份或云端数据带回当前设备。

数据管理页以三张平铺功能卡片呈现日常操作：「本地备份」卡片里的「创建本地备份包」生成加密的 `.flow.grano`，适合整机迁移或恢复；「卡片盒」卡片提供 grano 卡片盒导入、Anki 导入说明弹窗、跳转到卡片盒列表的「导出当前卡片盒」、当前卡片缓存占用与清空缓存入口；「加密密钥」卡片管理设备密钥。`.deck.grano` 只处理选定卡片盒、卡片和可打包的本地图片媒体，不创建任务本体，也不能替代完整本地备份。破坏性清理入口单独放在页面底部的「危险操作」分组。卡片盒与备份的区别阅读 [卡片盒、导入与导出](/manual/review-cards/decks-import-export/)。

恢复前，先确认备份来源、账号状态、密钥和版本条件。详情阅读 [备份与恢复](/manual/data-security-and-recovery/backup-and-restore/)。

## 订阅入口

订阅入口用来查看 GranoFlow Pro 权益、购买状态、恢复购买说明，以及不同平台购买可能带来的限制。
...
AI 助手入口用来选择或配置你要配合 GranoFlow 使用的外部 AI 工具，例如把整理好的内容交给 ChatGPT、Codex、Claude、Gemini、DeepSeek 或自定义助手处理。
...
标签管理用来创建、重命名、整理或停用任务标签。标签可以帮助你按场景、地点、精力或主题横向整理任务。
