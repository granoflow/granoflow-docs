...
数据管理页以三张平铺功能卡片呈现日常操作：「本地备份」卡片里的「创建本地备份包」生成加密的 `.flow.grano`，适合整机迁移或恢复；「卡片盒」卡片提供 grano 卡片盒导入、Anki 导入说明弹窗、跳转到卡片盒列表的「导出当前卡片盒」、当前卡片缓存占用与清空缓存入口；「加密密钥」卡片管理设备密钥。`.deck.grano` 只处理选定卡片盒、卡片和可打包的本地图片媒体，不创建任务本体，也不能替代完整本地备份。破坏性清理入口单独放在页面底部的「危险操作」分组。
