...
如果你在设置页看到账号、同步、数据、订阅、AI 助手、标签管理或 Pro 设置入口，可以把它们当成通往专门页面的按钮：账号管登录和设备，同步管多设备记录一致，数据管理管本地备份、卡组和危险操作，订阅管 Pro 权益，AI 助手和标签管理分别管外部 AI 工具配合方式和任务标签。
...
同步状态通常通过账号、设置里的同步入口，或顶部/侧栏的同步提醒进入。顶部/侧栏入口只在有待同步、正在同步、失败或需要处理时出现；没有待同步内容时，不会常驻显示“已同步”的完成图标。它用来让任务、项目、回顾等核心记录在多台设备之间保持一致。
...
数据管理页以功能卡片呈现日常操作：「本地备份」卡片里的「创建本地备份包」生成加密的 `.flow.grano`，适合整机迁移或恢复；「卡组」卡片提供 grano 卡组导入、Anki 导入说明弹窗、跳转到卡组列表的「导出卡组」、当前卡片缓存占用与清空缓存入口。`.deck.grano` 只处理选定卡组、卡片和可打包的本地图片媒体，不创建任务本体，也不能替代完整本地备份。破坏性清理入口单独放在页面底部的「危险操作」分组。卡组与备份的区别阅读 [卡组、导入与导出](/manual/review-cards/decks-import-export/)。
