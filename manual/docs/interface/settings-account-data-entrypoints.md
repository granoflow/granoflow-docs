...
数据管理页以三张平铺功能卡片呈现日常操作：「本地备份」卡片里的「创建本地备份包」生成加密的 `.flow.grano`，适合整机迁移或恢复；「卡片盒」卡片提供 grano / Anki 导入，以及跳转到卡片盒列表的「导出当前卡片盒」；「加密密钥」卡片管理设备密钥。`.deck.grano` 卡片盒包内的卡片正文以明文 JSON 保存，不会像本地备份那样用设备密钥加密；它只处理选定卡片盒及其卡片，不创建任务本体。破坏性清理入口单独放在页面底部的「危险操作」分组。
