...
这里的 CLI 只面向用户本机和运行中的桌面 App，不包含开发、构建、云端管理员、测试账号或发布类命令。

如果你只是手动使用 `granoflow help`、`granoflow version`、`granoflow status --json` 或 `granoflow open <route> --json`，通常不需要额外设置。需要让脚本或 AI 助手读取结构化结果时，优先使用 `--json`。
