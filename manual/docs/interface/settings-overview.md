...
如果你不知道某个设置会影响哪里，先看它属于哪个分组：外观和当前设备通常只影响这台设备的显示和提醒；账号、同步、数据、订阅和 AI 入口会带你进入对应页面，处理更具体的设置或说明。
...
- [Markdown 内容](/manual/interface/markdown-content/)
- [账号、同步与数据入口](/manual/interface/settings-account-data-entrypoints/)
- [命令行工具](/manual/desktop/command-line-tool/)

设置页是 GranoFlow 的统一入口。它把显示体验、当前设备偏好、账号、同步、数据、订阅、AI 和关于信息放在一个地方，但每个入口影响的范围不一样。
...
当前设备偏好控制这台设备上的操作习惯，例如计时器声音、应用锁、消息与提醒，以及滑动操作通知。
...
远程 Markdown 资源提醒也属于当前设备偏好。它控制远程图片、音频、视频和第三方视频链接首次加载前是否弹出确认，不会改写 Markdown 内容本身，也不会同步到其他设备。需要了解写法和限制时，阅读 [Markdown 内容](/manual/interface/markdown-content/)。
...
设置页提供“命令行工具”入口，用于安装或修复 `granoflow` 命令，并确认当前平台是否可以在终端里调用 CLI。
...
CLI 的 `backup create` 和 `backup restore` 也需要运行中的桌面 App 承接。备份恢复前应先使用 `--preview` 查看摘要，只有明确 `--confirm` 后才会导入。
...
- 想在描述或卡片字段里添加表格、公式或远程媒体，阅读 [Markdown 内容](/manual/interface/markdown-content/)。
