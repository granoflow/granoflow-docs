...
如果你只是手动使用 `granoflow help` 或 `granoflow version`，通常不需要额外设置。需要让外部脚本或 AI 自动化调用 Granoflow 时，可以在这个页面开启 Token 验证并创建最多 5 个 CLI Token。Token 原文只会在创建或重新生成时显示一次，关闭弹窗后不会再次显示。
