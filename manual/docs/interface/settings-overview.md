...
当前设备偏好控制这台设备上的操作习惯，例如计时器声音、应用锁、任务提醒横幅、滑动操作通知，以及防止投入时间段重叠。
...
CLI 的 `backup create` 和 `backup restore` 也需要运行中的桌面 App 承接。备份恢复前应先使用 `--preview` 查看摘要，只有明确 `--confirm` 后才会导入。完整命令见 [命令行工具](/manual/desktop/command-line-tool/)。
