---
title: "设置总览"
description: "了解 GranoFlow 设置页的分组方式，以及哪些入口只影响当前设备，哪些会进入账号、同步、数据、订阅或 AI 相关页面。"
---
...
- [命令行工具](/manual/interface/settings-overview/#命令行工具)
...
设置页提供“命令行工具”入口，用于管理 Granoflow CLI 的安装状态、系统脱敏复用和 Token 验证。

如果你只是手动使用 `granoflow help`、`granoflow version`、`granoflow status --json` 或 `granoflow open <route> --json`，通常不需要额外设置。需要让外部脚本或 AI 自动化调用 Granoflow 时，可以在这个页面开启 Token 验证并创建最多 5 个 CLI Token。Token 原文只会在创建或重新生成时显示一次，关闭弹窗后不会再次显示。

“使用系统脱敏功能”默认开启。开启后，CLI 输出以及未来导出给外部工具的数据会复用 GranoFlow 现有脱敏规则；如需修改脱敏词条，请从页面里的“管理脱敏设置”进入现有脱敏设置页。

CLI 的测试辅助入口只面向本机自动化和截图准备。它们需要运行中的桌面 App 承接命令，不会把云端管理员、订阅、通知或测试账号重置能力放进普通用户入口。

CLI 的 `export`、`import` 和 `backup` 命令也需要运行中的桌面 App 承接。导入、导出和备份会写入或读取本机数据文件；备份恢复前应先使用 preview 查看摘要，只有明确 confirm 后才会导入。
...
- 想让终端、脚本或 AI 自动化调用 Granoflow，进入设置里的“命令行工具”。
