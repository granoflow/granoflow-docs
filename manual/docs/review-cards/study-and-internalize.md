...
已内化不是 AI 自动评价，也不是说它永远不用复习。它只是一个很有用的提示：这条经验已经走出卡组，回到了你的行动里。
...
如果队列里暂时没有需要处理的卡片，页面会显示完成状态或引导你回到卡片统计。它不代表卡组里没有卡片，只说明当前主动复习范围内没有待练习项。任务详情、日回顾、周回顾、月回顾里的上下文复习仍可能显示相关卡片。
...
卡片的已归档和回收站都在「卡片管理」里查看。进入卡片管理后，点工具栏里的「筛选」，在状态里选择「已归档」或「回收站」。如果你是从某个卡组进入卡片管理，同一个筛选只会作用在当前卡组范围内。外层的「已归档」和「回收站」页面只处理任务，不再放卡片入口。
...
下一章会处理卡组、导入、导出和备份边界：当卡片变多之后，怎样迁移、整理，而不把卡组误当成完整备份或 Anki 克隆。
