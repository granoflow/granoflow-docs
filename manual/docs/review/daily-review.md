...
“心流时间”是另一项单独记录的时间。它表示你主观确认的真正专注时间，只能手工输入，不会从任务开始时间、完成时间或投入时间自动推导。你可以在日回顾里填写，也可以从当天已完成任务的详情里填写；同一天的已完成任务看到的是同一个日级心流时间。
...
如果某个任务的时间不准，先回到这条已完成任务的详情，点击“时间记录”，把开始时间和完成时间改成更接近真实情况的时间段。这样做会让日回顾里的任务时间块、今日投入时间，以及之后的周回顾、月回顾更接近你真实的一天。完整说明见[完成后先校准时间](../tasks/completion-and-archive#完成后先校准时间)。
...
周回顾和月回顾会把每天手工记录的心流时间累加，用来展示这一周或这一月真正进入专注状态的总量。
