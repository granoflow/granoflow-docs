...
在任务详情的节点行右侧点击更多按钮，可以选择“上移”“下移”“缩进”“提级”或“删除”。桌面端也可以用节点左侧的拖拽手柄调整顺序或层级；键盘焦点在节点列表上时，可以用 `Alt+↑ / Alt+↓` 上下移动，用 `Alt+← / Alt+→` 提级或缩进，用 `Delete` 或 `Backspace` 删除当前节点。

在手机或平板上，长按节点行会打开同一组节点操作。删除节点后，系统会暂时隐藏这个节点和它的子节点，并在底部提示里提供“撤销”。
