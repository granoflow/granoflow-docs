...
在桌面端，可以用节点左侧的拖拽手柄调整顺序或层级；键盘焦点在节点列表上时，可以用 `Alt+↑ / Alt+↓` 上下移动，用 `Alt+← / Alt+→` 提级或缩进。

要删除节点，点击节点行右侧的删除按钮；键盘也可以用 `Delete` 或 `Backspace` 删除当前节点。如果这个节点下面还有子节点，删除按钮不会显示，需要先删除或移动子节点。删除后，系统会暂时隐藏这个节点，并在底部提示里提供“撤销”。
