...
| 项目 | 可选 | 设置任务归属；如果仍然没有日期，它也会继续出现在收集箱 |
...
:::tip[善用自然语言输入]
在快速添加任务时，你可以直接写日期词、`#标签名`、`@项目或里程碑`、`~提醒时间`。比如输入 `整理报告 明天 #工作 @论文初稿 ~下午3点`，GranoFlow 会把明天的日期、“工作”标签、项目或里程碑候选和提醒时间显示出来，等你确认后再写入对应字段。详细规则见[用自然语言写任务](title-parser)。
:::
...
- **没有日期** → 进入收集箱
- **有日期** → 出现在那一天的任务列表里
- **有项目或里程碑** → 归属到对应项目；如果仍然没有日期，也会继续出现在收集箱
- **在项目页面里创建** → 直接归属到那个项目；是否进入任务列表仍取决于有没有日期
...
任务详情不是只用来改标题。它也是你把一件事从“记下来”推进到“正在做”的地方：

- 顶部可以修改标题，选择或更换项目、里程碑。
- 属性区域可以调整截止日期、提醒、标签，也可以添加图片。
- 描述区域适合放背景、链接、草稿或需要保留的说明。
- 节点区域用来把任务拆成步骤。
- 底部的“专注”和“完成”决定这条任务接下来进入哪种状态。

如果任务还没有开始，详情底部会同时显示“专注”和“完成”。点“专注”时，GranoFlow 会把这条任务设为当前正在做的任务，并开始一次专注记录；点“完成”则直接把它标为完成。如果这条任务已经在专注中，详情底部只会保留“完成”，因为此时最重要的动作是结束这次专注并完成任务。

这里容易误解：专注不是给任务加一个普通标签，而是告诉 GranoFlow“我现在就在做这件事”。所以同一时间如果已经有另一条任务正在专注中，当前详情页会提示你先完成或停止那条任务，再开始新的专注。
