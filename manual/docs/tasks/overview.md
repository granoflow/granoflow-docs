...
如果任务没有日期，它会先进入**收集箱（Inbox）**。项目和里程碑只说明任务归属，不会单独把一条无日期任务移出收集箱。你可以把收集箱理解成临时便条区：先放进去，有空再处理。

左上角菜单里可以找到这些任务视图：

| 入口 | 显示的内容 |
| --- | --- |
| 收集箱 | 没有日期、且仍是待办或进行中的任务 |
| 任务列表 | 正在推进的任务 |
| 已完成 | 已经做完的任务 |
...
GranoFlow 不鼓励你把未来每一天都排成精确日程。现实生活里，变化和意外才是常态；把下周、下个月的每一天都提前塞满，往往只会制造一种“计划已经失败”的压力。

所以任务列表只把今天和明天当作真正需要细排的日级计划：今天决定现在先做什么，明天留下一个清晰但不沉重的下一步。更远的任务，只需要估计大概会落在本周、本月、下个月或更晚，让它们形成一个粗略分布。等时间靠近，再把其中真正要处理的事情挪到今天或明天。

这和传统 Todo List 不一样：GranoFlow 不是让你提前控制未来每一天，而是帮你在不确定中保持方向感。
