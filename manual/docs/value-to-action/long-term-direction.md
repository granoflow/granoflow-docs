...
价值观不是“等我状态好了再考虑的事”，而是在你不确定、困难、混乱、甚至想逃避的时候，仍然能指向行动的方向。更多背景可以读 [ACT 与《幸福的陷阱》](/value-to-action/act-loop/)。
...
页面会先给出几个常见领域作为起点。它们只是建议，你可以直接改名，也可以在它们下面添加价值观；如果需要自己的方向，可以使用默认领域下方的“新建领域”按钮。你自己添加的领域会排在这个按钮下面。

<!-- manual-screenshot:id=value-domains-management -->
![先确定长期方向 界面截图](../../screenshots/zh-cn/value-domains-management.png)

如果截图没有加载，也不影响理解。你可以把这个页面想成一个“方向整理页”：每个领域下面都有自己的价值观列表；页面会用一些提示问题，帮助你把抽象念头写成一句完整的话，而不是只留下一个漂亮但难用的词。

每个领域卡片都有 AI 辅助入口。它会复制当前领域的名称、已有价值观和保存上限，让外部 AI 只围绕这个领域做开放式访谈。AI 可以帮你讨论这个领域会容纳哪些项目、哪些价值观更符合本心，也可以把参考书当作讨论材料；它不会替你决定方向。最终保存哪些内容，仍以你在领域页中的判断为准。
...
- [项目与里程碑：把长期方向拆成阶段目标](/value-to-action/projects-and-milestones/)：把持续投入放进一个能推进的容器。
- [任务与收集箱：把下一步写下来](/value-to-action/tasks-and-inbox/)：把阶段落到今天能做的一步。
- [回顾：让经历真正沉淀](/value-to-action/review-reflection/)：在真实行动后，慢慢修正和确认你的方向。
