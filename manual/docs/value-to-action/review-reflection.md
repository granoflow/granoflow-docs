...
进展页的「卡片学习」区域会显示主动复习卡片数和今日待复习数量，不包含已归档卡片。点击总卡片数可以进入卡片统计；卡片统计是卡片盒系列的主页面，左上角保留主菜单，并提供进入卡片复习、卡片管理和已归档卡片视图的入口。卡片复习和卡片管理会从统计页作为子页面打开，左上角提供返回入口。

如果想集中整理卡片，可以从统计页进入卡片管理，也可以直接进入已归档卡片视图，分别查看主动复习和已归档卡片，搜索、筛选、排序、编辑、归档、取消归档，或把不再需要的卡片移到回收站。

在卡片管理页，卡片盒相关操作放在正文筛选工具区的「卡片盒」菜单里，而不是页面顶部导航栏。桌面端可以从这里导入 Anki/APKG 卡片盒。当前导入只承接卡片内容：图片和音频附件会落到本机媒体资产；视频卡组默认不开放，只在测试或内测构建中开启。卡组里引用远程媒体时，GranoFlow 不会静默下载或保留远程链接；少量远程媒体会让你先查看链接并选择清除后导入，过量远程媒体会直接拒绝。完整的 Anki 模板、牌组配置、复习历史无损迁移仍不属于当前能力。
...
![卡片复习界面截图](../../screenshots/zh-cn/review-card-study-answer.png)
