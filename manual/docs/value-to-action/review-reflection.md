...
桌面端也可以在卡片管理里导入 Anki/APKG 卡片盒。当前导入只承接卡片内容：图片和音频附件会落到本机媒体资产；视频卡组默认不开放，只在测试或内测构建中开启。卡组里引用远程媒体时，GranoFlow 不会静默下载或保留远程链接；少量远程媒体会让你先查看链接并选择清除后导入，过量远程媒体会直接拒绝。完整的 Anki 模板、牌组配置、复习历史无损迁移仍不属于当前能力。
