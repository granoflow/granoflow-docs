---
title: "AI 如何輔助整理標題"
description: "輸入任務標題時，AI 可以識別日期、標籤、項目 / 里程碑提及和提醒，並以建議形式顯示；只有你確認後先會寫入任務。"
---

如果你想將「聽日下午三點同張總開會 #工作」呢類標題快速整理成任務資料，可以直接將完整句子輸入標題欄；GranoFlow 會嘗試從標題識別時間、標籤、項目 / 里程碑提及和提醒，再顯示成建議，等你確認後先會套用到任務。

確認並加入之後，任務會按實際欄位出現在收集箱或任務頁。截圖展示的是一個帶 `#release` 的示例任務在任務頁裏的結果狀態。

<!-- manual-screenshot:id=ai-title-parsing-quick-add -->
![AI 如何輔助整理標題 界面截圖](../../../screenshots/zh-hk/ai-title-parsing-quick-add.png)
...
- **標籤**：#工作 #個人 呢類井號標籤
- **項目 / 里程碑提及**：通過 `@` 或「加入項目」入口選擇已有項目下的可用里程碑；快速添加任務時，唔會只將任務掛到項目本身
