...
如果你保存的是第一次同步時抄錄的數據密鑰，它通常就是這套雲端數據的雲端同步密鑰。如果你後來改過數據密鑰，要用當前雲端同步數據對應的那一把。某個本地備份的數據密鑰只有在它和雲端同步保護本來就是同一串時，才能用於新設備同步。
...
![在新設備同步已有雲端數據 界面截圖](../../../screenshots/zh-hk/data-sync-device-join.png)

這個頁面通常會從帳號／設定中的同步入口，或頂部／側欄的同步異常提醒進入。它不是普通同步按鈕，也不是「已同步」狀態提示，而是在問你要保留哪一邊的數據。
...
先檢查有沒有複製完整，尤其是開頭、結尾和空格。確認你輸入的是雲端同步密鑰，不是賬號密碼，也不是隨手新產生的本地數據密鑰。備份數據密鑰只有在它和當前雲端同步保護本來就是同一把時才適用。
