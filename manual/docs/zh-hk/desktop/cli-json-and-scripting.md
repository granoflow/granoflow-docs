---
title: "JSON 輸出、環境變數與直接 HTTP 調用"
description: "理解 --json、--input <file|->、stdin、HTTP 端點與環境變數，便于腳本和 AI 自動化調用。"
---
...
- 寫操作前先加 `--dry-run`
- token 只從環境變數、配置文件案或安全凭據存储讀取，不寫進倉庫
- 不調用 `scripts/anz` 做用戶資料自動化；它是工程腳本入口
