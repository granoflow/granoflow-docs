...
description: "用 Think in workflows 的方式，把 CLI 命令和 HTTP 調用組合成可審查、可回滚的日常自動化流程。"
---
...
建立任務、更新回顧這類寫操作，优先用 JSON 文件保存輸入，再用 `--dry-run` 預覽：
...
`--dry-run` 返回的是本機請求預覽，不會調用真實寫入端點。它适合放在腳本的審查阶段，让
...
- 全程优先 `--json`
- 開始阶段先調用 `/v1/health` 或 `granoflow health --json`
...
理解工作流之後，再看 JSON、環境變數和直接 HTTP 調用，會更容易把腳本寫成可復用的小工具，
而不是一次性的終端命令串。
