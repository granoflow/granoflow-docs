...
數據管理頁以三張平鋪功能卡片呈現日常操作：「本地備份」卡片入面嘅「建立本地備份包」會生成加密嘅 `.flow.grano`，適合整機遷移或恢復；「卡片盒」卡片提供 grano / Anki 匯入、跳轉到卡片盒列表嘅「匯出目前卡片盒」、目前卡片緩存用量同清空緩存入口；「加密密鑰」卡片管理裝置密鑰。`.deck.grano` 只處理選定卡片盒、卡片同可打包嘅本地圖片媒體，唔會建立任務本體，亦唔可以替代完整本地備份。破壞性清理入口單獨放喺頁面底部嘅「危險操作」分組。
