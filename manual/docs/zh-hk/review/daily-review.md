...
「心流時間」是另一項單獨記錄的時間。它代表你主觀確認的真正專注時間，只能手動輸入，不會由任務開始時間、完成時間或投入時間自動推導。你可以在日回顧裏填寫，也可以從當日已完成任務的詳情裏填寫；同一日的已完成任務看到的是同一個日級心流時間。
...
如果你想重新梳理當日任務復盤，可以點「回顧今日任務」，讓 AI 按已記錄的任務時間理解當日脈絡，並整理當日涉及的領域、項目和里程碑推進。任務時間只作為只讀上下文，真實時間修正需要在任務列表或任務詳情裏手動完成。你把結果複製回 GranoFlow 後，仍需要在確認框確認，才會寫入任務標題、任務回顧、當日領域日報或可選新任務。完整流程見[回顧今日任務](../ai-assistance/daily-task-review)。

週回顧和月回顧會把每日手動記錄的心流時間累加，用來展示這一週或這一月真正進入專注狀態的總量。
