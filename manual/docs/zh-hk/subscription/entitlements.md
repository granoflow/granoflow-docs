---
title: "訂閱權益"
description: "會員權益都在哪裡，以及會員專屬設定和一般設定有什麼分別。"
---

如果你想知道自己可唔可以用某個會員功能，請先去該功能本身的入口查看：GranoFlow 沒有單獨一個「會員專區」。會員權益會出現在同步、AI 輔助、個人化設定等相關功能的位置。

會員狀態以伺服器返回嘅主會員能力為準。Pro 需要仍然喺有效期內；天使會員係長期主會員權益，普通到期唔會令佢失效。只有退款、轉移等會撤銷權益嘅情況，先會令天使會員不再可用。

<!-- manual-screenshot:id=subscription-vip-settings -->
![訂閱權益 界面截圖](../../../screenshots/zh-hk/subscription-vip-settings.png)

## 會員專屬功能

以下功能需要會員權益先可以完整使用。

### 同步

- 多裝置雲端同步
- 同步歷史與狀態查看

### AI 輔助

- AI 標題解析（識別日期、標籤、提醒）
- 剪貼簿助手
- AI 脫敏詞自訂

### 個人化

- AI 助手設定
- AI 研究偏好
- 日記、週記、價值觀和工作 / 學習日報 AI 輔助的研究取向自訂
- 診斷配置與熱力圖閾值設定

AI 改寫同回顧相關入口會讀取會員設定入面的「AI 研究偏好」。這個入口只編輯研究取向，不開放系統 prompt、導入格式或 JSON 結構。

## 非會員狀態下會怎樣

非會員通常仍然可以見到大部分會員專屬入口。見到入口，唔代表你已經有權限使用。

你可能會遇到兩種情況：

- 點擊入口後，App 顯示升級提示。
- 某些設定可以查看，但會變成唯讀，不能儲存修改。

這樣做是為了讓你知道功能在哪裡，以及訂閱後可能會解鎖哪些設定。

## 同步權益的特別說明

同步是會員專屬功能。如果目前帳號沒有可用權益，同步入口會提示你查看或開通會員。

<!-- manual-screenshot:id=subscription-sync-vip-upsell -->
![訂閱權益 界面截圖](../../../screenshots/zh-hk/subscription-sync-vip-upsell.png)

看到同步權益說明頁，**不代表同步已經開始，亦不代表你的本機資料已經遺失**。本機資料獨立於同步權益存在。

:::note[權益以伺服器為準]
App 本機顯示的權益狀態，來源於伺服器返回的帳號資訊。網絡不佳時可能會暫時顯示不正確，稍後重新整理即可。
:::
