...
| 描述 | 可選 | 放背景資料、連結、備註等補充內容；支援 [富文本內容](/manual/zh-hk/interface/markdown-content/) |
...
| 任務卡片 | 可選 | 在項目任務裏關聯或新增卡片，把這次任務裏的經驗保存成之後可複習的判斷 |
...
- **在項目頁面裏建立** → 直接歸屬到那個項目；是否進入任務列表仍取決於有沒有日期
...
如果任務屬於項目，詳情裏還會出現「任務卡片」區域。你可以從這裏新增卡片、關聯已有卡片，或進入這條任務相關卡片的練習。已關聯卡片會按筆記分組顯示；同一篇筆記下的多張卡片會放在一起，未歸檔卡片排在已歸檔卡片前面。取消關聯只會解除目前任務和這篇筆記下卡片的關係，不會刪除卡片本身。

任務描述、任務回顧，以及其他支援長文本的欄位可以使用富文本編輯。需要加入表格、公式、本地圖片、遠程音頻或 YouTube 影片時，閱讀 [富文本內容](/manual/zh-hk/interface/markdown-content/)。
:::caution[注意]
