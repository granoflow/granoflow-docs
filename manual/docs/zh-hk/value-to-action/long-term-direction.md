...
任務越来越多，項目一个接一个，日程也排得很满。可如果這些行動背後没有长期方向，生活就很容易變成一种持续运转的忙碌。

這也是為什么，在 GranoFlow 裡，“領域”和“價值觀”不是装饰性的設置项。它們不是讓你写一份漂亮的人生宣言，而是帮助你在任務和項目之外重新看见：

- 我长期在意什么？
- 我希望自己怎样行動？
- 我最近做的這些事，真的靠近了那个方向吗？

如果你想把长期目標變成今天能做的事，先不用急着整理一整套人生规划。你可以先用領域记录长期方向，再用價值觀说明自己希望如何行動，之後再把它們慢慢連接到項目、裡程碑、任務和回顧。
...
如果说任務回答的是“現在做什么”，項目回答的是“這段時间推进什么”，那么領域回答的是：
...
價值觀不是“等我状态好了再考虑的事”，而是在你不确定、困难、混乱、甚至想逃避的時候，仍然能指向行動的方向。更多背景可以读 [ACT 與《幸福的陷阱》](/zh-hk/value-to-action/act-loop/)。
...
領域页用来维护长期方向和價值觀，不是必须一次填完的分类系統。你可以从首页引导、侧栏管理入口或相关設置进入領域管理，然後逐个領域改名、调整副標題，并添加、修改或删除几条價值觀。

页面会先给出几个常见領域作为起点。它们只是建议，你可以直接改名，也可以在它们下面添加價值觀；如果需要自己的方向，可以使用默认領域下方的“新建領域”按钮。你自己添加的領域会排在这个按钮下面。

<!-- manual-screenshot:id=value-domains-management -->
![先确定长期方向 界面截圖](../../../screenshots/zh-hk/value-domains-management.png)

如果截图没有加载，也不影响理解。你可以把這个页面想成一个“方向整理页”：每个領域下面都有自己的價值觀列表；页面会用一些提示问题，帮助你把抽象念头写成一句完整的话，而不是只留下一个漂亮但难用的词。

每個領域卡片都有 AI 輔助入口。它會複製當前領域的名稱、已有價值觀和保存上限，讓外部 AI 只圍繞這個領域做開放式訪談。AI 可以幫你討論這個領域會容納哪些項目、哪些價值觀更符合本心，也可以把參考書當作討論材料；它不會替你決定方向。最終保存哪些內容，仍以你在領域頁中的判斷為準。
...
你可以先繼續使用任務、項目和回顧。等你积累了一些真实记录，再回头看：
...
- [項目與裡程碑：把长期方向拆成階段目標](/zh-hk/value-to-action/projects-and-milestones/)：把持续投入放进一个能推进的容器。
- [任務與收集箱：把下一步寫下来](/zh-hk/value-to-action/tasks-and-inbox/)：把階段落到今天能做的一步。
- [回顧：讓经历真正沉淀](/zh-hk/value-to-action/review-reflection/)：在真实行動後，慢慢修正和确认你的方向。
