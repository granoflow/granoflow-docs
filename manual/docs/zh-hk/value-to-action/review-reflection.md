...
手工「添加卡片」會先進入**筆記頁**：須先填寫標題（其餘欄位隨後解鎖）；標題與欄位內容會自動保存，保存成功時標題旁會短暫顯示「已保存」，無需「保存筆記」按鈕。已保存的筆記亦可以清空標題以重新鎖定其餘欄位。填寫標題與正文等 note 欄位後，再進入**卡片布局頁**，把 note 欄位編排到正面和反面；完成布局後會回到筆記頁。本 note 下還沒有卡片時，頁腳顯示「製作卡片」；已有卡片時只顯示「再排一張」，用於為同一 note 繼續製作 sibling 卡片。筆記頁「本筆記下的卡片」以正面縮略圖網格展示；點一張縮略圖可先預覽正反面，再點牌面進入布局編輯。進階選項內可編輯標題譯文、內容譯文，並添加自訂欄位。來源會在首次保存標題時自動寫入當前任務名，但不在筆記頁展示；複習時在卡片背面底部以弱樣式常顯。任務卡片區會按 note 成組展示 sibling 卡片；組級可以編輯筆記或取消關聯，卡級可以編輯布局、封存或移到回收站。
...
如果一張卡片或筆記已經有譯文，卡片詳情會把譯文放在進階區域；複習時若正面或背面旁出現譯文切換按鈕，可以點按在正文下方顯示或隱藏對應譯文。複習時先看問題，再顯示答案，最後用「遺忘、勉強、記得、輕鬆」四檔給自己一個簡單回饋。顯示答案後，如果這張卡片已經不適合繼續學習，可以移到回收站；如果只是想令佢不再進入主動複習，可以選擇封存。封存卡片仍可能在相關任務和回顧上下文中出現，並會標記「已封存」。這兩個動作都會提供撤銷入口。
...
在卡片管理頁，正文工具區只保留搜尋、篩選同排序；卡片盒選擇放在「篩選」面板入面。卡片盒級匯入、匯出同 Anki 匯入嘅主入口喺卡片盒列表（由卡片統計進入，或者喺卡片管理頁點麵包屑「卡片盒」）：列表頂有「匯入卡片盒」同「匯入 Anki 卡片盒」，每個頂層卡片盒行尾有匯出按鈕。設定 → 數據管理亦提供相同嘅 grano 同 Anki 匯入；「匯出目前卡片盒」會跳轉到卡片盒列表，由你喺列表揀要匯出嘅盒子。卡片管理頁唔再提供卡片盒匯入或匯出。確認匯入 Anki 卡片盒後會進入全螢幕進度頁，分階段顯示讀取卡片盒、匯入媒體、匯入筆記同匯入卡片；手機端進度頁底部會始終顯示固定弱提示，建議體積較大的卡片盒在電腦端匯入。當前 Anki/APKG 只支援匯入、不支援匯出；匯入只承接卡片內容：圖片和音訊附件會落到本機媒體資產；影片卡片盒預設不開放，只在測試或內測構建中開啟。卡片盒引用遠端媒體時，GranoFlow 不會靜默下載或保留遠端連結；少量遠端媒體會讓你先查看連結並選擇清除後匯入，過量遠端媒體會直接拒絕。完整的 Anki 模板、牌組配置、複習歷史無損遷移仍不屬於當前能力。

如果只是想在 GranoFlow 之間分享或遷移一個卡片盒，可以使用 GranoFlow 自有的 `.deck.grano` 卡片盒包。佢同完整本地備份 `.flow.grano` 唔同（卡片正文以明文 JSON 保存，唔會好似本地備份咁用裝置密鑰加密），亦唔取代 Anki/APKG：匯出時只能選擇頂層卡片盒，會自動帶上佢嘅子卡片盒同未刪除卡片；學習記錄預設不包含，只有你在匯出設定入面開啟「包含學習記錄」先會寫入。匯出需要一個 `GF1-DESK-...` 格式嘅版權 token，token 用來確認再次匯出時嘅持有關係，唔係防複製或強 DRM。
