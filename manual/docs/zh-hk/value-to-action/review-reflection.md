...
手工「添加卡片」會先進入**筆記頁**：填寫標題、正文、來源和關鍵詞等 note 字段並保存後，再進入**卡片布局頁**，把 note 字段編排到正面和反面；完成布局後會回到筆記頁，你可以用「再排一張」為同一 note 繼續製作 sibling 卡片。任務卡片區會按 note 成組展示 sibling 卡片；組級可以編輯筆記或取消關聯，卡級可以編輯布局、封存或移到回收站。來源和關鍵詞不會參與正面 / 反面編排，而是在卡片複習的背面底部以弱樣式常顯。
