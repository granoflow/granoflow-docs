...
手工「添加卡片」會先進入**筆記頁**：須先填寫標題（其餘欄位隨後解鎖）；標題與欄位內容會自動保存，無需「保存筆記」按鈕。填寫標題與正文等 note 欄位後，再進入**卡片布局頁**，把 note 欄位編排到正面和反面；完成布局後會回到筆記頁，你可以用「再排一張」為同一 note 繼續製作 sibling 卡片。來源會在首次保存標題時自動寫入當前任務名，並在進階選項裡只讀展示；關鍵詞仍可在進階選項編輯。任務卡片區會按 note 成組展示 sibling 卡片；組級可以編輯筆記或取消關聯，卡級可以編輯布局、封存或移到回收站。來源和關鍵詞不會參與正面 / 反面編排，而是在卡片複習的背面底部以弱樣式常顯。
