...
這張表只保存你手動維護的固定替換規則。系統依規則自動發現的電子郵件、金額、連結、日期、長數字、信用卡、IBAN、電話等內容，會依 AI 去識別化設定裡的類別預設策略暫時替換成 `EMAIL_1`、`MONEY_1`、`ID_1` 這類短期代號，不會自動寫進這裡的長期詞表。
...
- 可能漏掉縮寫、別名、錯字或其他變體寫法。
- 只處理你已經設定的固定詞；規則型自動發現會使用短期代號，不會替你維護長期詞表。
