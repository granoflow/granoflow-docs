...
這張表只保存你手動維護的固定替換規則。系統依規則自動發現的電子郵件、金額、連結、日期、長數字、信用卡、IBAN、電話等內容，會依 AI 去識別化設定裡的類別預設策略暫時替換成更容易讀懂的短期去識別化值，例如 `13xxxxx4821` 或 `foxxxx3920@1846.com`，不會自動寫進這裡的長期詞表。
...
- 可能漏掉縮寫、別名、錯字或其他變體寫法。
- 只處理你已經設定的固定詞；規則型自動發現會使用短期去識別化值，不會替你維護長期詞表。
