...
如果你儲存的是第一次同步時抄錄的資料密鑰，它通常就是這套雲端資料的雲端同步密鑰。如果你後來改過資料密鑰，要用目前雲端同步資料對應的那一把。某個本機備份的資料密鑰只有在它和雲端同步保護本來就是同一串時，才能用於新裝置同步。
...
這個頁面通常會從帳號／設定中的同步入口，或頂部／側欄的同步異常提醒進入。它不是一般的同步按鈕，也不是「已同步」狀態提示，而是在問你要保留哪一邊的資料。
...
先檢查有沒有複製完整，尤其是開頭、結尾和空格。確認你輸入的是雲端同步密鑰，不是帳號密碼，也不是隨手新產生的本機資料密鑰。備份資料密鑰只有在它和目前雲端同步保護本來就是同一把時才適用。
