...
- 選擇「取消」會停止這次處理。你可以先回到舊裝置、雲端概覽或備份頁面核對資料。

無論選哪條路，都不能保證未上傳成功的本機附件、另一台裝置上的未同步改動，或沒有密鑰的資料一定能復原。做選擇前，先確認目前裝置和舊裝置上最重要的資料還能看到。

## 雲端資料概覽

如果帳號裡已經有雲端歷史資料，GranoFlow 可能先顯示雲端資料概覽。你可以用它粗略判斷：雲端裡有多少任務、專案、里程碑、附件，最近什麼時候更新過，以及資料涵蓋的大致時間範圍。

<!-- manual-screenshot:id=data-cloud-data-overview -->
![在新裝置同步既有雲端資料介面截圖](../../../screenshots/zh-tw/data-cloud-data-overview.png)

這個頁面常見於已登入帳號的同步入口，尤其是目前裝置還沒啟用上傳能力、但雲端已經有可下載歷史資料時。它的主要動作是「下載雲端資料」。在部分可以上傳、風險更高的場景裡，頁面也可能出現「清空雲端資料」。

- 「下載雲端資料」是一次復原動作，不等於自動開啟日常雲端同步。下載後，先回到任務、專案和回顧頁面檢查內容。
- 「清空雲端資料」會要求再次確認，並可能要求系統驗證。確認後，雲端同步資料會被清空；不要把它當成重新整理或重新載入。
- 如果頁面提示本機和雲端加密狀態不同，先去「加密與復原密鑰」處理密鑰問題，不要反覆嘗試下載。

雲端概覽只能幫你判斷雲端歷史資料的大致範圍。它不能保證每個附件都已經完整下載到目前裝置，也不能替你判斷另一台裝置上是否還有未同步內容。
