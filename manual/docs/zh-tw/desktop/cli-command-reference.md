...
`granoflow` CLI 是本機 HTTP API 的用戶端。讀寫 App 資料的命令會呼叫
`http://127.0.0.1:<port>` 下的公開端點；離線備份包轉換命令則只處理你提供的備份包
檔案，不連接 App。

如果你不想安裝 CLI，也可以直接用 curl 呼叫相同的 HTTP API。
...
`config` 會顯示 CLI 目前使用的 API 位址、token 來源和設定檔案路徑。token 會被脫敏。
...
`--input <file|->` 表示從檔案或 stdin 讀取 JSON。`--dry-run` 只生成本機請求預覽，不會
呼叫寫入端點。
...
這兩個命令只轉換備份包檔案，不建立 App 備份，也不把資料恢復到 App。
