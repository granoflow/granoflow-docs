...
任務可以歸屬到專案和里程碑；整理到專案時，通常需要選擇具體里程碑。專案或里程碑只說明任務屬於哪裡，不會單獨讓一條沒有日期的任務離開收集箱。專案可以封存或完成；如果專案裡還有未處理的任務，系統會先讓你決定這些任務怎麼處理。
...
沒有日期、而且狀態是待辦或進行中的任務，會出現在收集箱。專案和里程碑只是歸屬；如果任務仍然沒有日期，它也可以繼續留在收集箱。一旦你給任務安排了日期，或者完成、封存、刪除它，它才會離開收集箱。你可以把收集箱想成口袋裡的便條紙：先放進去，之後再整理。
...
金鑰很重要。建立備份或首次同步時看到的「資料密鑰」、新裝置同步時輸入的「雲端同步密碼」，本質上都在保護加密資料，只是目標不同。遺失金鑰，就解不開舊備份或對應的加密雲端資料。GranoFlow 會多次提醒你儲存金鑰，但伺服器不能替你找回遺失的金鑰。

### 備份與還原

備份是把裝置上的全部資料匯出為 `.flow.grano` 檔案，並用金鑰加密保護。

還原是把這個備份檔案帶回目前裝置，需要提供建立備份時使用的金鑰。如果附件在備份時沒有完全下載，備份裡可能不會包含完整附件。
