...
資料管理頁以三張平鋪功能卡片呈現日常操作：「本機備份」卡片裡的「建立本機備份包」生成加密的 `.flow.grano`，適合整機遷移或還原；「卡片盒」卡片提供 grano 卡片盒匯入、Anki 匯入說明彈窗、跳轉到卡片盒列表的「匯出目前卡片盒」、目前卡片快取佔用與清空快取入口；「加密金鑰」卡片管理裝置金鑰。`.deck.grano` 只處理選定卡片盒、卡片和可打包的本機圖片媒體，不建立任務本體，也不能替代完整本機備份。破壞性清理入口單獨放在頁面底部的「危險操作」分組。卡片盒與備份的區別閱讀 [卡片盒、匯入與匯出](/manual/zh-tw/review-cards/decks-import-export/)。
