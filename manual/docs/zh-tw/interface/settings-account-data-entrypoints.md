...
資料管理頁以三張平鋪功能卡片呈現日常操作：「本地備份」卡片裡的「建立本地備份包」會產生加密的 `.flow.grano`，適合整機遷移或恢復；「卡片盒」卡片提供 grano / Anki 匯入，以及跳轉到卡片盒列表的「匯出目前卡片盒」；「加密金鑰」卡片管理裝置金鑰。`.deck.grano` 卡片盒包內的卡片正文以明文 JSON 保存，不會像本地備份那樣用裝置密鑰加密；它只處理選定卡片盒及其卡片，不會建立任務本體。破壞性清理入口單獨放在頁面底部的「危險操作」分組。
