...
有些卡片屬於一則更完整的筆記。練習頁會提供筆記入口，開啟後可以在側邊面板查看：
...
關聯專案會用三個圓點表示覆蓋 0、1、2、3 個及以上專案；沒有專案的任務會放在「未歸入專案」下面。點擊關聯任務時，GranoFlow 會先關閉筆記面板，再開啟任務詳情。

這個面板的作用不是讓你在練習時讀長文，而是在需要時幫你找回上下文：這張卡從哪裡來，和哪些任務有關，同一則筆記下還有哪些卡片。
...
卡片管理會按筆記組織卡片，而不是把每張卡都當成孤立條目。你可以從一篇筆記下面看到它的多張卡片、封存狀態和學習狀態；已經內化的卡片在整理時會有額外提醒，避免你誤把已經在多個專案中證明有用的經驗移走。
