...
「心流時間」是另一項單獨記錄的時間。它表示你主觀確認的真正專注時間，只能手動輸入，不會從任務開始時間、完成時間或投入時間自動推導。你可以在日回顧裡填寫，也可以從當天已完成任務的詳情裡填寫；同一天的已完成任務看到的是同一個日級心流時間。
...
週回顧和月回顧會把每天手動記錄的心流時間累加，用來展示這一週或這一月真正進入專注狀態的總量。
