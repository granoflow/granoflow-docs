...
![建立與編輯任務 介面截圖](../../../screenshots/zh-tw/tasks-create-edit-dialog.png)
...
| 描述 | 選填 | 放背景資訊、連結、備註等補充內容；支援 [富文本內容](/manual/zh-tw/interface/markdown-content/) |
...
| 任務卡片 | 選填 | 在專案任務裡關聯或新增卡片，把這次任務裡的經驗保存成之後可複習的判斷 |

如果你選擇的自訂標籤帶有範本，而且這條任務還沒有描述和節點，GranoFlow 會在儲存後把範本內容加入任務描述和根節點。任務已經有描述或節點時，範本會跳過，避免覆蓋你已經填寫的內容。同一個標籤範本對同一條任務只會自動加入一次；如果範本加入失敗，標籤選擇會保留，你可以之後手動補充描述或節點。
...
- **在專案頁面裡建立** → 直接歸屬到那個專案；是否進入任務清單仍取決於有沒有日期
...
![建立與編輯任務 介面截圖](../../../screenshots/zh-tw/tasks-detail-review-editable.png)

任務完成或封存後，詳情裡會顯示「任務回顧」。你可以在這裡補充這件事實際花了多久、後來確認了什麼、下次要注意什麼。如果你先完成任務並寫了回顧，之後又把任務恢復為未完成，已有回顧不會被清空；任務再次完成或封存後，回顧會重新顯示並可以編輯。

如果任務屬於專案，詳情裡還會出現「任務卡片」區域。你可以從這裡新增卡片、關聯已有卡片，或進入這條任務相關卡片的練習。已關聯卡片會按筆記分組顯示；同一篇筆記下的多張卡片會放在一起，未封存卡片排在已封存卡片前面。取消關聯只會解除目前任務和這篇筆記下卡片的關係，不會刪除卡片本身。

任務描述、任務回顧，以及其他支援長文本的欄位可以使用富文本編輯。需要加入表格、公式、本機圖片、遠端音訊或 YouTube 影片時，閱讀 [富文本內容](/manual/zh-tw/interface/markdown-content/)。
:::caution[注意]
