...
領域页用来维护长期方向和價值觀，不是必须一次填完的分类系統。你可以从首页引导、侧栏管理入口或相关設定进入領域管理，然後逐个領域改名、調整副標題，并添加、修改或删除几条價值觀。
...
![先确定长期方向 介面截圖](../../../screenshots/zh-tw/value-domains-management.png)

如果截图没有加载，也不影响理解。你可以把這个页面想成一个“方向整理页”：每个領域下面都有自己的價值觀列表；页面会用一些提示问题，帮助你把抽象念头写成一句完整的话，而不是只留下一个漂亮但难用的词。

每個領域卡片都有 AI 輔助入口。它會複製目前領域的名稱、已有價值觀和保存上限，讓外部 AI 只圍繞這個領域做開放式訪談。AI 可以幫你討論這個領域會容納哪些專案、哪些價值觀更符合本心，也可以把參考書當作討論材料；它不會替你決定方向。最終保存哪些內容，仍以你在領域頁中的判斷為準。
